...
export const edit_coll = (id, title, old_title) => async dispatch => {
    CollectionAPI.update(id, title)
    .then(res => {
        dispatch({
...
export const new_cat_collection = (notebook, note, title, key) => async dispatch => {
    NoteAPI.add_cat(note, key)
    .then(res => {
        dispatch({
