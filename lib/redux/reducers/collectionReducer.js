...
export const collectionReducer = (state = initialState, action ) => {
    switch (action.type) {
        ...
        case types.NEW_NOTE_COLLECTION:
            ...
            newBook[action.title] = action.note;
            return {
                ...state,
        ...
        case types.EDIT_NOTE_COLLECTION:
            newBook = state[action.notebook];
            if (action.oldTitle !== action.title) delete newBook[action.oldTitle];
            newBook[action.title] = action.note;
            return {
                ...state,
                [action.notebook] : newBook,
        ...
        case types.NEW_CAT_COLLECTION:
            newBook = state[action.notebook];
            if (newBook[action.title].keywords.indexOf(action.cat === -1)) newBook[action.title].keywords.push(action.cat);
            return {
                ...state,
                [action.notebook] : newBook,
            }
        case types.DEL_CAT_COLLECTION:
            let keywords = state[action.notebook][action.title].keywords;
